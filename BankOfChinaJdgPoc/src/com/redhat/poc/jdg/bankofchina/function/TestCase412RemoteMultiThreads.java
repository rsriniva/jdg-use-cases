...
/**
 * 数据加载测试 (client-server mode, 2个JDG缓存节点,复制模式)
 * 读取/Users/maping/Code/cmcccachetest/data/test/data/userinfo/目录下csv文件
 * 并Put到Cache中
 *
 * @author maping
 */
public class TestCase412RemoteMultiThreads extends Thread {
    ...
    private static int csvFileEnd = 594;
    private int fileName;

    public TestCase412RemoteMultiThreads(int fileName) {
...
//            System.out.println("####### " + Thread.currentThread().getName() + " : userinfo 缓存中的记录数是 " + cache.size());
            ...
            Logger.getLogger(TestCase412RemoteMultiThreads.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
