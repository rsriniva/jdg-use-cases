...
public class SimpleHotRodQueryClient {
	...
	public static void main(String[] args) throws ProtoSchemaBuilderException,
		...
		SearchFactoryImplementor sf = (SearchFactoryImplementor) Search.getSearchManager(cache).getSearchFactory();

		// 'Entity' is the indexed object's class name
		DirectoryBasedIndexManager dm = (DirectoryBasedIndexManager) sf.getIndexManagerHolder().getIndexManager(Entity.class.getName());
		RAMDirectory directory = (RAMDirectory) dm.getDirectoryProvider().getDirectory();
		long indexSize = directory.sizeInBytes();
	}
